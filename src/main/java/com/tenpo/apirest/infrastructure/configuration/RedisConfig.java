...
public class RedisConfig {
    ...
    public CacheManager cacheManager(RedissonClient redissonClient) {
        var configs = Map.of(
                CacheConstants.HISTORY_CACHE_NAME, new CacheConfig(),
                CacheConstants.OTHER_CACHE_NAME, new CacheConfig()
        );
        return new RedissonSpringCacheManager(redissonClient, configs);
    }

    @CacheEvict(cacheNames ={
            CacheConstants.HISTORY_CACHE_NAME,
            CacheConstants.OTHER_CACHE_NAME
    }, allEntries = true)
